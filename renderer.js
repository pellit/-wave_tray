...
function Renderer(scale) {
  ...
  this.waterMesh = GL.Mesh.plane({ detail: 200, size: 2 * (this.scale || 1.0) });
  this.waterShaders = [];
  for (var i = 0; i < 2; i++) {
  ...
  this.sphereMesh = GL.Mesh.sphere({ detail: 10, radius: 0.125 * (this.scale || 1.0) });
  // Mesh para solo el piso
  this.floorMesh = GL.Mesh.plane({ detail: 200, size: 2 * (this.scale || 1.0) });
  // Renderizar solo el piso (sin paredes)
  Renderer.prototype.renderFloorOnly = function () {
    water.textureA.bind(0);
    this.tileTexture.bind(1);
    this.causticTex.bind(2);
    // Dibujar solo el piso horizontal y= -1
    gl.pushMatrix();
    gl.translate(0, -1, 0);
    gl.rotate(-90, 1, 0, 0);
    // Usar un shader simple para el piso
    if (!this.floorShader) {
      this.floorShader = new GL.Shader(
        'varying vec2 vUv;\n' +
        'void main() {\n' +
        ...
        'varying vec2 vUv;\n' +
        'void main() {\n' +
        '  gl_FragColor = texture2D(tiles, vUv) * vec4(0.3, 0.3, 0.3, 1.0);\n' +
        '}\n'
      );
    }
    this.floorShader.uniforms({ tiles: 1 }).draw(this.floorMesh);
    gl.popMatrix();
  };
  ...
  ');
  this.cubeMesh = GL.Mesh.cube({ size: 2 * (this.scale || 1.0) });
